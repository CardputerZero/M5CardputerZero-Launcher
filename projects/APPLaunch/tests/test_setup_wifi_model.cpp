...
int main()
{
    ...
    assert(model.cursor_position() == 6);
    assert(model.move_cursor_left());
    assert(model.cursor_position() == 5);
    assert(model.append("X"));
    assert(model.password() == "secreXt");
    assert(model.move_cursor_right());
    assert(model.cursor_position() == 7);
    assert(model.erase_last());
    assert(model.password() == "secreX");
    assert(model.masked_display() == "******_");
    ...
    assert(ssid.cursor_position() == 11);
    assert(ssid.move_cursor_left());
    assert(ssid.move_cursor_left());
    assert(ssid.append("_"));
    assert(ssid.ssid() == "Hidden Wi_Fi");
    assert(ssid.move_cursor_right());
    assert(ssid.erase_last());
    assert(ssid.ssid() == "Hidden Wi_i");
    ssid.reset();
    assert(ssid.append("Hidden WiFi"));
