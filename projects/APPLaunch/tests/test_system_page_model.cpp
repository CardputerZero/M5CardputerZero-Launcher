...
int main()
{
    ...
    assert(update_job_label(0, "running") ==
           "Checking launcher update...\nLauncher may restart");
    assert(update_job_label(1, "failed:incompatible:1") == "No compatible update");
    assert(update_job_label(1, "failed:version-not-newer:1") ==
           "Launcher is up to date");
    assert(update_job_label(1, "failed:download-package:1") ==
