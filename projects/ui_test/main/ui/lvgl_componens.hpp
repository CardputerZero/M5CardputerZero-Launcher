...
void lvgl_bind_event(lv_obj_t *obj,
                            lv_event_code_t event_code,
                            void *user_data,
                            Fun &&fun)
{
    LvglBindContext *context = new LvglBindContext{
            std::function<void(lv_event_t *)>(std::forward<Fun>(fun)),
            user_data
        };
...
public:
    ...
    void SetSize(int32_t w, int32_t h) const { lv_obj_set_size(ComponensObj, w, h); }
    void OnEvent(lv_event_code_t event_code, std::function<void(lv_event_t *)> &&fun, void *user_data) const {
        DComponens::lvgl_bind_event(ComponensObj, event_code, user_data, std::forward<std::function<void(lv_event_t *)>>(fun));
    }
    ...
    virtual void AnimateNextIn(std::function<void()> AnimateOverFunc) = 0;
    virtual void AnimateNextOut(std::function<void()> AnimateOverFunc) = 0;
...
public:
    ...
    void create_ui(lv_obj_t * parent) override
    {
        ...
        DComponens::lvgl_bind_event(slider, LV_EVENT_VALUE_CHANGED, NULL, std::bind(&LvExampleGetStarted4::slider_event_cb, this, std::placeholders::_1));
