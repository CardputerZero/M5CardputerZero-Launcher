...
struct SettingEntry {
    ...
    SettingEntry(const std::string &name, SettingApiCallBack callback, bool enable_icon) : label(name), Componens_api(std::move(callback)), icon_enabled(enable_icon)
    {
    }
    SettingEntry(const std::string &name, SettingApiCallBackFunc func, bool enable_icon) : label(name), icon_enabled(enable_icon)
    {
